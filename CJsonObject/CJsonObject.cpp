...
neb::CJsonObject* ReadJson(const std::string& filePath)
{
	...
	neb::CJsonObject* obj = new neb::CJsonObject();
	if (!obj->Parse(s))
	{
		delete obj;
		return nullptr;
...
bool ReadJson(const std::string& filePath, neb::CJsonObject* placementMemory)
{
	...
	std::string s(len + 1, ' ');
	ifs.read((char*)s.data(), len);
	new (placementMemory)neb::CJsonObject();
	if (!placementMemory->Parse(s))
	{
		placementMemory->~CJsonObject();
		return false;
	}
	return true;
}
