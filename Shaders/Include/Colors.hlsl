...
//
float3 ApplyLut3D(Texture3D<float4> tex, SamplerState samplerTex, float3 uvw, float2 scaleOffset)
{
    uvw.xyz = uvw.xyz * scaleOffset.yyy * scaleOffset.xxx + scaleOffset.xxx * 0.5;
    return tex.SampleLevel(samplerTex, uvw, 0).rgb;
}
...
//
float3 ApplyLut2D(Texture2D<float4> tex, SamplerState samplerTex, float3 uvw, float3 scaleOffset)
{
